...
class Agent():
    def __init__(self, adhocnet, flow_id, reward_type):
        ...
        if os.path.exists(self._model_path):
            ...
            self._main_net.load_state_dict(torch.load(self._model_path, map_location=DEVICE))
            self._target_net.load_state_dict(torch.load(self._model_path, map_location=DEVICE))
        else:
    ...
    def get_closest_neighbors(self, available_bands):
        ...
        min_vals = np.min(self.adhocnet.nodes_on_bands[available_bands,:], axis=0)
        fully_occupied_mask = (min_vals==1)
        if np.any(fully_occupied_mask):
            fully_occupied_neighbors = np.where(fully_occupied_mask)[0]
        else:
            fully_occupied_neighbors = np.array([])
