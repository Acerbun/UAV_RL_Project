...
def route_random(agent):
    ...
    available_nodes_mask = (agent.adhocnet.nodes_on_bands[band]==0)
    available_nodes = np.where(available_nodes_mask)[0]
    ...
    while True: # reproduce it as a sequence of actions by the agent
        ...
        if next_hop in closest_neighbors:
            action_index = np.where(closest_neighbors == next_hop)[0][0] if np.any(closest_neighbors == next_hop) else -1
